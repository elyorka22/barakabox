...
export function AdminStoreImageUpload({ storeId, kind, label, currentUrl, onUploaded }: Props) {
  ...
  const upload = async (file: File) => {
    ...
    try {
      const response = await fetch(
        `${getApiBaseUrl()}/admin/marketplace/stores/${storeId}/image?kind=${kind}`,
        {
          method: 'POST',
    ...
    <div className="space-y-2 rounded-xl border border-dashed border-slate-300 p-3">
      ...
      <div
        className={`relative overflow-hidden rounded-lg bg-slate-100 ${
          kind === 'banner' ? 'aspect-[2/1]' : 'aspect-square max-w-[120px]'
        }`}
      >
        {preview ? (
      ...
      <button
        ...
        className="w-full rounded-lg border border-slate-200 py-2 text-xs font-medium text-slate-700 disabled:opacity-50"
      >
        {uploading ? 'Yuklanmoqda...' : preview ? 'Almashtirish' : 'Rasm yuklash'}
      </button>
