...
const TABS: { id: PickerTab; label: string; icon: typeof ClipboardList }[] = [
...
export function PickerBottomNav({ tab, onTab, activeCount = 0 }: Props) {
  return (
        ...
        {TABS.map((item) => {
          ...
          return (
            <button
              ...
              className={`relative flex min-h-[56px] flex-col items-center justify-center gap-0.5 ${active ? 'text-[#16A34A]' : 'text-[#9CA3AF]'}`}
            >
